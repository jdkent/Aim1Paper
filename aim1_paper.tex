...
\begin{flushleft}
{\Large
\textbf\newline{A comparson of beta series correlation methods to detect condition dependent functional connectivity} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
\section*{Abstract}
Research utilizing cognitive tasks drive discovery in neuroscience by connecting specific cognitive functions
with patterns of brain activity.
The most common application for tasks completed during functional magnetic resonance imaging (fMRI)
examines activity of individual brain regions.
...
each other during different task-dependent conditions.
These region to region correlations represent brain networks for the conditions.
Two common methods to generate beta series are Least Squares All (LSA) and
Least Squares Separate (LSS).
However, LSS and LSA have not been compared with simulations and participant data
...
experimental designs and using participant data.
Our simulations suggest LSS outperforms LSA, but participant data
does not show an advantage using LSS.
Overall, results demonstrate the importance of validation using both simulations
and participant data and impacts how future studies should design tasks
...
The single event estimates become difficult to estimate when events occur close together~\ref{fig:introhrf}.
...
5mm spheres were drawn around each statistical peak (21 peaks total)~\ref{fig:methroimap}.
...
in the null data to measure the specificity of each method.
...
The contrasts performed on the top 20 CNR ROIs in the Schaefer Atlas show qualitatively similar results,
...
$switch - repeat$.
% create a consistent name for the top 20 CNR ROIs
The results with respect to the null data for the Top 20 CNR ROIs in the schaefer atlas demonstrate
...
\begin{figure}[H]
  ...
  \caption{
    Visualization of the significant ROI-ROI pairs detected in \ref{fig:real-schaeferbest}
...
the significant ROI-ROI pairs found with LSS are largely non-overlapping with LSA~\ref{fig:non-overlap}.
...
We provided the first in depth comparison between LSS and LSA to detect 
...
the results from the participant data do not show a clear winner.
...
mirroring the suggestion on the number of trials needed to detect activation~\cite{Huettel2001}.
...
contrast illustrated in the main result\ref{fig:main-result}.
...
which would make the processing pipeline between simulated and participant data closer.

\subsection*{Participant Data Conclusions}
\label{discussion:taskswitching-conclusions}

The participant data suggests LSS and LSA perform similarly.
...
The table suggests model misfit may be a driving factor for spurious results in the Schaefer atlas~\ref{table2}.
