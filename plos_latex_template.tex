...
each other during different task conditions within an MRI scanner.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{contrast_summary}
  \caption{
...
%     \item Activation Atlas
%     \begin{itemize}
%       \item Task versus Null: LSS (n=36; p<0.001) $==$ LSA (n=31; p=0.001)
%       \item Switch versus Single: LSS R:(n=11; p=0.581) N:(n=15; p=0.223) $==$ LSA R:(n=12; p=0.477) N:(n=14; p=0.359)
%       \item Repeat versus Single: LSS R:(n=19; p=0.047) N:(n=13; p=0.331)$=<$ LSA (n=24; p=0.005) N:(n=11; p=0.58)
%       \item Switch versus Repeat: LSS R:(n=7; p=0.958) N:(n=4; p=0.990) $=<$ LSA R:(n=16; p=0.102) N:(n=13; p=0.149)
%     \end{itemize}
%   \end{itemize}
%   \item All Participants (N=61)
%   \begin{itemize} 
%     \item Activation Atlas
%     \begin{itemize}
%       \item Task versus Null: LSS (n=24; p=0.001) $==$ LSA (n=23; p=0.003)
%       \item Switch versus Single: LSS R:(n=16; p=0.101) N:(n=10; p=0.605) $>=$ LSA R:(n=11; p=0.566) N:(n=13; p=0.270)
%       \item Repeat versus Single: LSS R:(n=19; p=0.033) N:(n=11; p=0.599) $==$ LSA R:(n=19; p=0.029) N:(n=13; p=0.347)
%       \item Switch versus Repeat: LSS R:(n=12; p=0.36) N:(n=5; p=0.982) $>=$ LSA R:(n=6; p=0.943) N:(n=17; p=0.025)
%     \end{itemize}
%   \end{itemize}
% \end{itemize}

% Using paired ttests and permutations changed the conclusions such that there does appear to be
% a difference between the detection power of LSS and LSA.
% Both appear to detect significant differences between a task and a null condition as well as
% $repeat - single$, but not $switch - single$ or $switch - repeat$.

% While simulations indicate LSS would be a more powerful method, the real data do not suggest an advantage.
% \begin{table}[!ht]
...
All of our simulations had AVNR greater than or equal to scan noise and we saw
LSS still significantly outperformed LSA for our measure of detection power.
We observed a discrepency of AVNR and noise coherency between our results and those of Abdulrahman \& Henson (2016)\cite{Abdulrahman2016}.
...
motion noise, physiological noise, drift, and signal autocorrelation.
However, all noise sources were given equal weighting, which is unlikely to be true with real data.
Future work should estimate the relative weightings of each component of noise
so they can be incorporated appropriately.
