...
In contrast, real data does not show a clear advantage of one method over the other.
...
For example, it could be the case that for half of the events of the same type two regions are active,
...
we simulated two voxels of fMRI time-series data and convolved short (0.2 second)
...
two voxel fMRI time-series containing drift, autocorrelation ($\rho$ = 0.5), physiological noise,
...
For all simulations the time of repetition was set to two seconds.
...
We chose average IEIs at two, four, six, and eight seconds to reflect a common range of IEIs
...
(varying between 0.0-0.9) with AVNR of either one or two.
The ground truth correlation was varied to ensure the ground truth correlations were above
and below the correlations of the noise time-series between the two voxels,
...
relative to the noise standard deviation to CNRs of one or two.
...
two eight minute resting state runs.
...
So if the task has three conditions, 
a single GLM would have four event regressors, one for the target
event, and three for the remaining conditions.
...
The schaefer atlas has qualitatively similar results.
...
1) Increasing the average IEI up to six seconds improves detection power between conditions substantially,
but increasing the average IEI above six seconds only offers modest improvement in detection power.
